...
CourbeBezier::CourbeBezier(int deg, int dim, Vecteur3d pol[])
{
    degre = deg;
    dimension = dim;
    ordre = deg + 1;
    poles = new Vecteur3d[ordre];
    if(pol == NULL) for(int i =0; i < ordre; i++) poles[i] = Vecteur3d(dim);
    else for(int i =0; i < ordre; i++) poles[i] = pol[i];
}
...
CourbeBezier::CourbeBezier(int deg, int dim, double P[][3])
{
    ...
    ordre = deg + 1;
    poles = new Vecteur3d[ordre];
    for (int i = 0; i < 3; i++) poles[i] = Vecteur3d(dimension, P[i]);
}
...
CourbeBezier::CourbeBezier(CourbeBezier const& b)
{
    ...
    ordre = degre + 1;
    poles = new Vecteur3d[ordre];
    for (int i = 0; i < ordre; i++) poles[i] = b.poles[i];
}
...
{
    Vecteur3d *P = new Vecteur3d[degre];
    for (int i = 0; i < degre; i++) P[i] = (1 - t)*poles[i] + t*poles[i+1];
    for (int i = degre-1; i >= 0; i--)
        for (int j = 0; j < i; j++) P[j] = (1 - t)*P[j] + t*P[j+1];
...
void CourbeBezier::dessine()
{
    ...
    gp<<"splot $POLES with points pt 7,"<<" $COURBE with lines\n";
}